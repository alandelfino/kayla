...
export function DerivationItemsSheet({ derivationId, derivationType }: { derivationId: number, derivationType: 'text' | 'color' | 'image' }) {
  ...
  const handleDragOver = (e: React.DragEvent, targetId: number) => { if (savingOrder) return; e.preventDefault(); if (hoveredId !== targetId) setHoveredId(targetId) }
  ...
  const { isPending: savingOrder, mutate: reorderMutation } = useMutation({
    mutationFn: async (reordered: DerivationItem[]) => {
      for (const it of reordered) {
        const response = await privateInstance.put(`/api:JOs6IYNo/derivation_items/${it.id}`, { value: it.value, order: it.order, derivation_id: derivationId })
              ...
              {(isLoading || isRefetching) ? (
                ...
                itemsLocal.length === 0 ? (
                  ...
                  <li
                    ...
                    className={`rounded-md border bg-background shadow-sm hover:shadow-md transition-shadow px-3 py-2 flex items-center gap-3 ${draggingId === i.id ? 'opacity-60 ring-2 ring-primary' : ''} ${hoveredId === i.id && draggingId !== i.id ? 'ring-1 ring-muted-foreground/30 bg-muted/30' : ''}`}
                  >
                    ...
                    {derivationType === 'color' ? (
                      <>
                        ...
                        />
                        <span className='flex-1 truncate font-medium'>{i.value}</span>
                      </>
